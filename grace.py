...
def load_images(directory,a_name, a_num): #animation Name, animation Number
...
blue_rect = blue[0].get_rect()
kblue_rect = kblue[0].get_rect()
still_rect = still[0].get_rect()
kstill_rect = still[0].get_rect()
...
while run:
    ...
    elif keys[pygame.K_d] and still_rect.right < mwidth * tilesize + 15: #RGIHT
        moveright = True
        still_rect.x += spd
